...
// Cascade Falls 마블 경로 시뮬레이션 기반 스폰 존 [minX, maxX, minY, maxY]
//
// 경로: 핀필드 → D1(우측출구 x=17,y=35) → D2(좌측출구 x=9,y=42)
//      → D3(우측출구 x=17,y=54) → D4(좌측출구 x=9,y=61) → 스피너 → 퍼널
//
// 존0,1,2 = 아이템 2개씩 / 존3~6 = 1개씩 (i%7, 총 10개)
const ITEM_SPAWN_ZONES = [
  [5,  21, 14, 22],  // 존0 (×2): 핀 필드 — 모든 마블이 통과하며 산란
  [17, 22, 35, 38],  // 존1 (×2): D1 우측 출구 — 마블이 D1 타고 x=17에 집중
  [4,  9,  42, 45],  // 존2 (×2): D2 좌측 출구 — D2 타고 x=9로 집중 후 낙하
  [17, 22, 54, 56],  // 존3 (×1): D3 우측 출구 — D3 타고 x=17에 집중
  [4,  9,  62, 70],  // 존4 (×1): D4 좌측 출구 긴 채널 — 스피너 전 16유닛 공간
  [5,  21, 79, 83],  // 존5 (×1): 메인 스피너 후 — 전 마블 통과
  [8,  18, 88, 94],  // 존6 (×1): 퍼널 접근 — 전 마블 중앙으로 수렴
];
const ITEM_COUNT = 10;
